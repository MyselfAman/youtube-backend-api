...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    if(title=="" && description === "") throw new ApiError(400, "Please provide title and description for video.")
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    const {title, description} = req.body;
    if(title === "" && description ==="" && videoId === "") throw new ApiError(400,"Please provide title, description or thumbnail to update")


    const thumbnailFilePath = req.file?.path;

    if(title === "" && description ==="" && !thumbnailFilePath) throw new ApiError(400,"Please provide thumbnail");

    const uploadedThumbnail = await uploadCloudinary(thumbnailFilePath);


    const video = await Video.findByIdAndUpdate(videoId,{
        $set:{title : title, description : description, thumbnail : uploadedThumbnail?.url}
    },{new:true})
...
const deleteVideo = asyncHandler(async (req, res) => {
    ...
    if(!videoDeleted) throw new ApiError(400,"Video is not deleted there is some problem try again");

    res.status(200).json(new ApiResponse(200,{videoDeleted},"Video deleted successfully."))
...
const togglePublishStatus = asyncHandler(async (req, res) => {
    ...
    if(publish === "") throw new ApiError(400,"Please provide publish status of the video");
    ...
    if(!video) throw new ApiError(400,"Publish is not updated please check data and video id.");

    res.status(200).json(new ApiResponse(200,{video},"Publised has been updated."));
})
